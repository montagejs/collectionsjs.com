...
for (var i = 0; i < collections.length; i++) {
    // use mousedown to get there as soon as possible before the page unloads
    collections[i].addEventListener("mousedown", handleSelect);
}
...
function handleSelect(event) {
    if (ga) {
        ga("send", "event", "chooser", "select", event.target.textContent);
    }
}
